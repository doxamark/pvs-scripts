...
class DouglasCountyPPScript extends BaseScript {
    async performScraping() {
        await this.page.goto(this.accountLookupString, { waitUntil: 'networkidle2' });
        console.log(`Navigated to: ${this.page.url()}`);

        await this.page.waitForSelector('input[value="Click Here"')
        await this.page.click('input[value="Click Here"')

        await Promise.race([
            this.page.waitForSelector('#taxAccountSummary', { visible: true }),
            this.page.waitForSelector('#middle', { visible: true })
        ]);

        const errorMessage = await this.page.$("#middle")
        if (errorMessage) {
            const error = await this.page.evaluate(el => el.textContent, errorMessage);

            if (error.trim().includes("The account could not be found")) {
        ...
        await this.page.waitForSelector("#taxAccountSummary")

        const links = await this.page.$$(".widget a")
        let yearLink = null
        for (let element of links) {
            const text = await this.page.evaluate(el => el.textContent, element);
            if (text.trim().includes(`Property Tax Notice ${this.year}`)) {
                yearLink = element;
                break;
            }
        }
        ...
        await this.page.waitForSelector("#myReports a")

        const printBtns = await this.page.$$('#myReports a');
        let printBtn = null;
...
export default DouglasCountyPPScript;

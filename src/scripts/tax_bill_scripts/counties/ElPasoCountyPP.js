...
class ElPasoCountyPPScript extends BaseScript {
    async performScraping() {
        await this.page.goto(this.accountLookupString, { waitUntil: 'networkidle2' });
        console.log(`Navigated to: ${this.page.url()}`);


        await this.page.waitForSelector("#criteria")
        await this.page.type("#criteria", this.account)

        await this.page.select('select', "5");


        await this.page.click("input[type=submit]")

        await this.page.waitForSelector("#addToCart")
        await this.page.click("#addToCart h3 a")
        ...
        for (let label of detailLabels) {
            ...
            if (labelText.trim().includes(`tax information for ${this.year}`)) {
                ...
                for (let linkLabel of linkLabels) {
                    const linkLabelText = await this.page.evaluate(el => el.getAttribute("href"), linkLabel);
                    if (linkLabelText && linkLabelText.trim().includes('currentstatement')) {
...
export default ElPasoCountyPPScript;

...
class WilliamsonCountyTXPPScript extends BaseScript {
    async performScraping() {
        ...
        await Promise.race([
            ...
            this.page.waitForSelector('#dnn_ctl03_dnnSkinMessage', { visible: true })
        ]);
    
        // Check if the error message is present and contains the specific text
        const errorMessages = await this.page.$$('#dnn_ctl03_dnnSkinMessage');
        ...
        const printLabels = await this.page.$$('.iconLink');
        let printLink = null
        for (let label of printLabels) {
            const labelText = await this.page.evaluate(el => el.textContent, label);
            if (labelText.trim().includes("Original Tax Bill")) {
                printLink = label
            }
        }
...
export default WilliamsonCountyTXPPScript;

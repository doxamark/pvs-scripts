...
class TravisCountyPPScript extends BaseScript {
    async performScraping() {
        await this.page.goto(this.accountLookupString, { waitUntil: 'networkidle2' });
        console.log(`Navigated to: ${this.page.url()}`);

        await this.page.waitForSelector("#qsfInput")
        await this.page.type("#qsfInput", this.account)

        await this.page.click("#qsfButtonSearch")

        // Wait for either the error message or the bill view to be visible
        await Promise.race([
            this.page.waitForSelector('#searchResultsTable', { visible: true }),
            this.page.waitForSelector('#globalError', { visible: true })
        ]);
					
        // Check if the error message is present and contains the specific text
        const errorMessages = await this.page.$$('#globalError');
        let noBillFound = false;
        for (let element of errorMessages) {
            const text = await this.page.evaluate(el => el.textContent, element);
            if (text.trim().includes('The search criteria you have provided did not return any results.')) {
                noBillFound = true;
                break;
            }
        }
        ...
        await this.page.waitForSelector("#searchResultsTable a")
        await this.page.click("#searchResultsTable a")

        await this.page.waitForSelector("#app-content")

        let targetYearMatch = false
        const detailLabels = await this.page.$$('h4');
        ...
        const printLinks = await this.page.$$('.link-simple.nav-link');
        let printLink = null
        for (let label of printLinks) {
            const labelText = await this.page.evaluate(el => el.textContent, label);
            if (labelText.trim().includes('CURRENT BILL')) {
                printLink = label
            }
        }
...
export default TravisCountyPPScript;
